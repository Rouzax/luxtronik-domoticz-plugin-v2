...
- Migrates Temperature, Meter (kWh), Percentage, and LightingLog (switches/selectors/text) tables
...
- Migration uses **Unit IDs** (not device names) - works regardless of language setting
- Only data older than the new device's first entry is migrated (no duplicates)
- Migrates: Temperature, Meter (kWh), Percentage, and LightingLog (switches/selectors/text) tables
- New devices (compressor runtime, discharge temp, etc.) start fresh with no historical data

## Files

| File | Description |
|------|-------------|
| `discover.sql` | Preview device mapping and data counts |
| `migrate.sql` | Perform the actual data migration |